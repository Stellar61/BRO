...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "http://127.0.0.1:3000",
        "http://127.0.0.1:3001",
        "https://bro-frontend-app.azurewebsites.net"  # Azure frontend
    ],
...
def optimize(req: OptimizeRequest):
    ...
    # Filter rows for the requested route number
    ...
    # Check total students for this route
    total_students = stops_for_route["students per stop"].sum()

    if total_students <= 15:
        return {
            "error": f"Route {req.route_no} skipped — only {total_students} students (≤15)"
        }
...
@app.get("/")
def home():
    return {"message": "Bus Route Optimizer (with student count filter) is running!"}
