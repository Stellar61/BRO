...
def optimize(req: OptimizeRequest):
    ...
    if stops_for_route.empty:
        ...
        sample_routes = available_routes[:50]
        return {
    ...
    # --- Remove duplicate first stop if present at the end ---
    if order[-1] == order[0]:
        order = order[:-1]
    ...
    for i in order:
        if i < len(stops_for_route):
            ...
            ordered_points.append({
                ...
                "Time": COLLEGE_TIME,  # Fixed 8 AM arrival
                "students": 0
            })
...
@app.get("/")
def home():
    return {"message": "Bus Route Optimizer backend is running with proper final stop at Rajalakshmi Engineering College (8:00 AM)!"}
